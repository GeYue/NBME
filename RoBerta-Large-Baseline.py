...
def assemble_preds_string(x):
    pred_text = ""
    #print (f"current==>{x['location2']}")
    for val in x['location2']:
        if pred_text == "":
            pred_text = f"\'{x['pn_history'][val[0]:val[1]]}\'"
        else:
            pred_text = pred_text + ";" + f"\'{x['pn_history'][val[0]:val[1]]}\'"
    return pred_text
...
for i in range(epochs):
    ...
    # evaluate model
    valid_loss, score, location_preds, location_preds_test = eval_model(model, test_dataloader, criterion)
    ...
    if valid_loss < best_loss:
        ...
        torch.save(model.state_dict(), "nbme_RoBERTa-Large.pth")
        #torch.save(model, "nbme_bert_model.model")

    fo = open (f"submission-F-{i}.txt", "w+")
    fo.write(f"id,location2,annotation_pred\n")
    X_test["location2"] = location_preds
    X_test["location_test"] = location_preds_test
    for idx in range(X_test['location2'].size):
        x_txt = assemble_preds_string(X_test.iloc[idx])
        x_id = X_test.iloc[idx]["id"]
        x_location2 = X_test.iloc[idx]["location2"]
        fo.write(f"{x_id},{x_location2},{x_txt}\n")
    X_test[["id", "location", "annotation"]].to_csv(f"submission-T-{i}.csv", index = False)
    fo.close()
